...
- `MathPreview`: native MathML rendering generated from the validated calculator AST.
...
- `HelpDialog`: supported notation and examples authored as Markdown and rendered at build time with `Bun.markdown`.
...
The calculator has no KaTeX dependency. Browser-side preview and saved rows use native presentation MathML generated from the trusted TypeScript AST, while solver result values use presentation MathML generated by SymPy. Bun's built-in Markdown renderer converts the static notation guide into filtered HTML before Vite serves or builds the application; Bun is not treated as a browser-side math typesetter.
